...
from pageobjects.schoolrolldetails_page import SchoolRollDetails
from testdatas import addschoolroll_data as AD
from testdatas import main_data as MD
class TestAddSchoolRoll:
    @pytest.mark.smoke
    @pytest.mark.usefixtures("login_driver")
    @pytest.mark.parametrize("addschoolroll",MD.addschoolroll_menu)
    #创建学籍参数化需要添加的数据
    @pytest.mark.parametrize("inputschoolroll",AD.input_addschoolrolldata)
    #login_driver夹具中返回的参数
    #断言数据
    @pytest.mark.parametrize("assert_data",AD.assert_data )
    #添加学籍点击暂存按钮
    def test_saveaddstudent(self,login_driver,addschoolroll,inputschoolroll,assert_data):
        MainPage(login_driver).click_left(addschoolroll["left"])
        MainPage(login_driver).click_firstmenu(addschoolroll["firstmenu"])
        MainPage(login_driver).click_lastmenu(addschoolroll["lastmenu"])
        ...
        MainPage(login_driver).out_iframe()
        assert assert_data["snum"] in querydata
    #添加学籍点击提交按钮
